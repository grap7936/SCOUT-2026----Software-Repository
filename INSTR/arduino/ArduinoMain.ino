...
// --- Hardware Pins ---
const int ENCODER_PIN_A = 2; // encoder interrupt pins for the motor encoder are set to be 5 and 6 rather than 2 and 3 which are the default
...
// Encoder instance utilizing the interrupt pins 5 and 6
Encoder encoder = Encoder(ENCODER_PIN_A, ENCODER_PIN_B, PPR); // Inputs: 1.) Digitial PWM/Interrupt pin plug in #1
...
void setup() {
  ...
  encoder.enableInterrupts(doA, doB); // enables changing interrupt pins from the default system pins of 2 and 3 to other pins. The inputs simply link the pin changes to the embedded interrupt functions doA and doB performed in the code.
  ...
  attachInterrupt(digitalInterrupt(ENCODER_PIN_A), doA, CHANGE);  // Hardware-links the external physical changes on Digital Pins 2 and 3 directly
  ...
  attachInterrupt(digitalInterrupt(ENCODER_PIN_B), doB, CHANGE);
...
void loop() {
    ...
    if (Data_Package.hasNewData) {
        ...
        if (!testModeActive && Data_Package.y >= 0) {
            ...
            //  Pass calculated parameters through the custom PID sequence to establish control voltage
            float setpoint = motor.shaft_angle + rad_diff;
