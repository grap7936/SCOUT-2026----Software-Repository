...
declare -A EXECS=(
  ...
  [IdleCam]="IdleCam"
  [DebrisTracking]="main"
  [TestDetector]="detectorTest"
  [TestGraph]="graphTest"
  [TestSelector]="selectorTest"
  [TestFull]="fullTest"
)
...
state_testcam() {
  ...
  log "TestCam active — taking sample image."
  run_exec TestCam
...
state_testalgorithm() {
  # run algorithm tests from low->high level
  log "TestAlgorithm active — running all code tests."
  log "Testing Detector code"
  run_exec TestDetector
  log "Testing Graph code"
  run_exec TestGraph
  log "Testing Selector code"
  run_exec TestSelector
  log "Testing Full code"
  run_exec TestFull

  log "Leaving TestAlgorithm — return to Standby."
}
