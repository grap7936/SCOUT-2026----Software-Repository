...
bool ArduinoSend::sendTargetCoordinates(uint16_t id, int16_t x, int16_t y, std::ofstream& logFile) {
    ...
    if (bytes_read > 0) {
        ...
        if (sscanf(read_buffer, "SUCCESS: Frame [%d] Pos: %lf", &parsedFrame, &parsedPos) == 2) {
...
void ArduinoSend::logTelemetry(std::ofstream& logFile, int frameNum, double motorPos, bool isMotorTest) {
    ...
    // CHANGED: Organizes variables into a clean structural column layout based on the data type source
    if (isMotorTest) {
    ...
    // CHANGED: Flushes the stream immediately so data hits your disk without waiting for cache boundaries
    logFile.flush();
}
