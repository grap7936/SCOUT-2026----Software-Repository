...
Code Summary: Main execution script to test bidirectional communication between a RaspberryPi/Jetson and an Arduino UNO over USB Serial.
              This is the main script that sends the specific test case commands as defined in "ReceiveEnd_Arduino.cpp" from the Jetson to
              the Arduino to trigger each test while in TESTING_MODE. Explicit details for each test are also sent to the console for reference
              in the displayCommandMenu() function below.
...
#include "ArduinoSend.hpp" // header file include to access all necessary functions from the ArduinoSend class
#include <iostream> // C++ standard library
#include <fstream> // enables input and output streams for data
#include <vector> // enables the vector type -- constantly changing array variables
...
void displayCommandMenu() {
    ...
    std::cout << "  Y = -7  ->  Exit the program and shut down the system" << std::endl;
...
int main() {
    ...
    std::string serial_port = "/dev/ttyCH341USB0";  // establish serial connection (likely syntax for the Arduino NANO) to pass into linux system 
                                                    // -- check this by plugging the NANO into your Jetson's USB port, open a terminal, and run-> Bash: ls /dev/tty* 
                                                    // to see what specific connection it is being detected as. Then, reconcile this line to be the same string to ensure correct serial connection.
   ...
   // Preallocate necessary variables for the testing loop
    ...
    // Uncomment these lines if you want to enable a specific RPM user input rather than some globally defined speed value
        ...
        std::string temp; // by initializing this temporary variable, this stores a garbage value and so by pressing any key after sentry mode is activated this then will stop sentry mode.
        std::cin >> temp; 
    ...
    case -2: {  // Test Motor Function by moving the motor in a full revolution of 360 degrees -- much more details for this function are on the "ReceiveEnd_Arduino.cpp" side

        // These statements are currently not being used but can be uncommented if wanting to implement a specific user input degree for the motor to move to. Note, if uncommented, the "ReceiveEnd_Arduino.cpp" motor_test() function must also be modified
        ...
        sender.sendTargetCoordinates(0, 0, -2); // 2nd input is the amount that the motor will move in degrees from its starting position
    ...
    case -3: {  // Ping function to test bilateral communications by sending a test byte of data back and forth between Jetson and Arduino
...
//  Give the Arduino a tiny fraction of a second to reply, then read the buffer ---
        usleep(50000); // 50ms delay to ensure the Arduino has time to process and print the response
        
        std::cout << "\n[PING RESPONSE]:\n";
        
        // Call the newly created string reading function
        std::string response = sender.readStringResponse(); 
        std::cout << response << "\n";
    ...
    case -4: { // PID controller function -- sends x-coordinate displacement to cause the PID_step function to move to target coordinates 3 times to observe overall overshoot and system functionality
        ...
        int target_angle = 90; // target angle of 90 degrees along the x-axis to move the motor using PID control
        ...
        for (int i = 0; i < 3; i++) { // loop to send the PID controller 3 different target coordinates to test the functionality of the PID controller and observe overshoot and system response time
            ...
            sender.sendTargetCoordinates(0, target_angle, -4); // sends a 90 degree x-coordinate displacement to the PID controller from its current position
            ...
            target_angle = target_angle * -1; // flips the target angle to -90 degrees to move the motor back to its original position using PID control for the next iteration
           }
    ...
    case -7: {
        ...
        return 0; // exit the program if the user inputs -7 to stop testing and tracking mode and shut down the system
